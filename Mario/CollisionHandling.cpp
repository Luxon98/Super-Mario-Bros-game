			...
			if (isPlayerJumpingOnMonster(player, *it)) {
				...
				if (dynamic_cast<Shell*>(*it)) {
					if (dynamic_cast<Shell*>(*it)->isActive()) {
						...
						Direction direction = (player->getX() >= dynamic_cast<Shell*>(*it)->getX() ? Left : Right);
				...
				if (dynamic_cast<Turtle*>(*it)) {
					...
					world.addCrushedCreature(Position(dynamic_cast<Creature*>(*it)->getX(), 
				...
				if (player->isImmortal()) {
					if (dynamic_cast<Turtle*>(*it) || (dynamic_cast<Shell*>(*it) 
						&& dynamic_cast<Shell*>(*it)->isActive())) {
